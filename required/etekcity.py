import requests, hashlib, json, time, pprint, random, logging
from .eswd16 import ESWD16
...
class EtekCity:
    ...
    def login(self):
        ...
        if self.email == '' or self.password == '':
            self.Logger.error ('Credentials are required!')
        else:
            ...
            if response_json['result'] is not None:
                if 'token' in response_json['result']:
                    ...
                    self.Logger.info ('Account ID: %s' % self.account_id)
    ...
    def get_time_zone(self):
        ...
        self.Logger.info ('Time Zone: %s' % self.time_zone)
            ...
            if response_json['result']['total'] > 0:
                for device in response_json['result']['list']:
                    if device['deviceType'] == 'ESWD16':
                        self.devices[len(self.devices)] = ESWD16(device, account_info)

        self.Logger.info ('Devices: ')
        for device in self.devices:
            self.Logger.info(self.devices[device].get_info())
